...
describe("material matrix interaction flow", () => {
  test("selecting a grade updates known material totals and heat intensity", () => {
    ...
    const targetGroup = groups.find(
      (group) =>
        group.moduleType === "Plasma Accelerator" &&
        group.name === "Short Range Blaster",
    )

    expect(targetGroup).toBeTruthy()
    if (!targetGroup) return

    const gradeOne = targetGroup.blueprints.find(
      (blueprint) => blueprint.Grade === 1,
    )
    expect(gradeOne).toBeTruthy()
    if (!gradeOne) return

    const byId = new Map(
      targetGroup.blueprints.map(
        (blueprint) => [blueprint.id, blueprint] as const,
      ),
    )
    const baseTotals = sumIngredientsForSelection(new Set(), byId)
    const selectedTotals = sumIngredientsForSelection(
      new Set([gradeOne.id]),
      byId,
    )
  ...
  test("search and type filters reduce visible groups without clearing selected totals", () => {
    ...
    expect(visibleBefore.some((group) => group.key === targetGroup.key)).toBe(
      true,
    )
    expect(
      visibleAfterEngineerHidden.some((group) => group.key === targetGroup.key),
    ).toBe(false)
